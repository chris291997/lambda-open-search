...
export class ChasesMapper {
  static map(data: any): ChaseExcerptV2 | undefined {
    ...
    }, ${data.providerProfile.address.city}, ${data.providerProfile.address.state}, ${data.providerProfile.address.zip}`;
    ...
    return {
      ...
      patientPhone: data.patient.contactDetails.find((contact: { type: string; value: string }) => contact.type === 'phone')?.value ?? "",
      ...
      facilityTin: data.practitioner?.tin,
  ...
  static mapArray(data: any[]): ChaseExcerptV2[] {
    return data
      .map((s) => this.map(s))
      .filter((s): s is ChaseExcerptV2 => s !== undefined);
  }
}
